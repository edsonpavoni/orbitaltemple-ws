/**
 * Send test emails using REAL data from CSV for edsonpavoni@gmail.com
 * This tests that the variables (name, daysAgo) are working correctly
 */
...
/**
 * Parse CSV and get records for test email
 */
function getTestRecords(filePath: string, testEmail: string, limit: number = 2): EmailRecord[] {
 ...
 */
async function sendTestEmail(record: EmailRecord): Promise<boolean> {
  try {
    ...
    console.log(`✅ Sent: "${record.name}" (${record.daysAgo} days ago, ${record.language})`);
    return true;
  } catch (error) {
 ...
 */
async function main() {
  console.log('🧪 TEST with REAL DATABASE records\\n');
  ...
  console.log(`📁 Loading real records for: ${TEST_EMAIL}\\n`);
  ...
  const records = getTestRecords(csvPath, TEST_EMAIL, 2);

  console.log(`✅ Found ${records.length} records for testing:\\n`);
  records.forEach((r, i) => {
    console.log(`   ${i + 1}. "${r.name}" - ${r.daysAgo} days ago (${r.language}) - ${r.submissionDate}`);
  });
  ...
  for (const record of records) {
    const success = await sendTestEmail(record);
  ...
  console.log('\\n✨ Test complete!\\n');
  console.log(`📊 Results: ${successCount}/${records.length} sent\\n`);
  console.log('📬 Check your inbox at edsonpavoni@gmail.com');
  console.log('   Verify that the name and days ago are correct!\\n');
}
